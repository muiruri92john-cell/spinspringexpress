// =====================================================
// routes/owner.js — Owner routes
// =====================================================

const express = require('express');
const router = express.Router();
const OwnerModel = require('../models/owner');
const { isOwner, isAuth } = require('../middleware/auth');
...
router.get('/login', (req, res) => {
  if (req.session.spinUser) {
    ...
    return res.redirect(role === 'owner' ? '/owner' : (role === 'attendant' ? '/attendant' : '/customer'));
  }
...
router.get('/owner', isOwner, ah(async (req, res) => {
  ...
  const [owner, stats, devices, attendants, customers, recentOrders] = await Promise.all([
